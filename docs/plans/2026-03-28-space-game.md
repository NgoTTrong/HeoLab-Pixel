# Astro Raid (Space Invaders) Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build "Astro Raid" — a Space Invaders game with multiple wave patterns, power-ups (Triple Shot, Shield, Screen Bomb), boss every 5 waves, and 3 lives at `/games/space`.

**Architecture:** Canvas-based rendering with `requestAnimationFrame`. Config-driven power-ups, wave patterns, boss phases. Lives system with pixel ship icons.
...
**Files:**
- Create: `src/games/space/config.ts`
...
**Commit:**
```bash
git add src/games/space/config.ts
git commit -m "feat: space game config"
```
...
**Files:**
- Create: `src/app/games/space/page.tsx`
...
git commit -m "feat: Astro Raid space invaders with waves, boss, power-ups"
```
...
**Files:**
- Create: `src/app/games/space/layout.tsx`
- Modify: `src/app/games/page.tsx` — set Astro Raid `available: true`
- Modify: `src/app/sitemap.ts` — add space URL
...
**Commit:**
```bash
git add src/app/games/space/layout.tsx src/app/games/page.tsx src/app/sitemap.ts
git commit -m "feat: space SEO metadata, mark as available"
```
