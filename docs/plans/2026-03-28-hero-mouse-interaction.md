# Hero Mouse Interaction — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add custom pixel cursor, emoji spring-physics scatter, and neon particle trail to the landing page hero section.

**Architecture:** All logic lives in a single `useEffect` in `src/app/page.tsx`. Physics run in a `requestAnimationFrame` loop using refs — no React re-renders during animation. Emojis' CSS `animation` removed; float oscillation moved into the RAF loop to avoid `transform` conflicts. Particle trail spawns fixed `<div>` elements into `document.body`, removed via `animationend`. Desktop only.

**Tech Stack:** Next.js 16 App Router, React `useRef`/`useEffect`, `requestAnimationFrame`, CSS keyframes, Tailwind CSS v4.

---

### Task 1: Add `particleFade` keyframe to globals.css

**Files:**
- Modify: `src/app/globals.css`

**Step 1: Add keyframe after `pulseRing`**
...
git commit -m "feat: add particleFade keyframe for hero particle trail"
```

---

### Task 2: Add mouse interaction to page.tsx hero section
...
type EmojiPhysics = {
  ...
  vx: number;
  vy: number;
  homeX: number; // viewport px at mount
  homeY: number;
};
...
**Step 2: Verify in browser (localhost:3000)**

Checklist:
- [ ] Custom crosshair cursor appears when hovering over hero, disappears when leaving
- [ ] Neon particle trail (green/pink/yellow dots) follows cursor
- [ ] Each floating emoji scatters away when cursor comes within ~130px
- [ ] Emojis spring back elastically to original position
- [ ] Float animation still works (sinusoidal Y movement)
- [ ] No console errors
- [ ] Outside hero section: default cursor restored, no particles
...
git commit -m "feat: hero mouse interaction — pixel cursor, emoji scatter, particle trail"
```
