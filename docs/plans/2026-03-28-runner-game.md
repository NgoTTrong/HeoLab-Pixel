# Pixel Dash (Endless Runner) Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build "Pixel Dash" — an endless runner with double-jump, unlockable characters (Dino/Robot/Ninja), world themes changing by score, and score multipliers at `/games/runner`.

**Architecture:** Canvas-based rendering with `requestAnimationFrame`. Config-driven characters, worlds, obstacles. Character selection UI before game starts.

**Tech Stack:** Next.js 16 App Router, TypeScript, React (useRef + useEffect + useState), HTML Canvas, Tailwind CSS v4.
...
**Files:**
- Create: `src/games/runner/config.ts`
...
git commit -m "feat: runner game config"
```
...
**Files:**
- Create: `src/app/games/runner/page.tsx`
...
export default function RunnerPage() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      ctx.font = `${Math.min(g.obstacles[0]?.width ?? 28, 36)}px serif`;
...
git commit -m "feat: Pixel Dash endless runner with characters, worlds, double jump"
```
...
**Files:**
- Create: `src/app/games/runner/layout.tsx`
- Modify: `src/app/games/page.tsx` — set Pixel Dash `available: true`
- Modify: `src/app/sitemap.ts` — add runner URL
...
**Commit:**
```bash
git add src/app/games/runner/layout.tsx src/app/games/page.tsx src/app/sitemap.ts
git commit -m "feat: runner SEO metadata, mark as available"
```
